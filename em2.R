#######EM algorithm
EM.Iter <-
  function(sdata,n.int = 5,order = 3, r1, max.iter = 500,cov.rate = 0.1){
    # sdata <- mdata; n.int = 5;order = 3; r1 = 0; max.iter = 500;cov.rate = 0.1
    Xp<-cbind(sdata$X1,sdata$X2)
    M<-ncol(Xp)
    Kn<-n.int+order
    N<-nrow(sdata)
    ##设置EM算法初值
    ...
    ## 回归系数beta1的迭代矩阵,每一行是一个迭代结果
    bt1em = matrix(0,nrow = max.iter, ncol = M)
    ## 样条系数gamma的迭代矩阵, 每一行是一个迭代结果
    gmem = matrix( 0, max.iter, Kn )
    ## 回归系数beta2的迭代矩阵,每一行是一个迭代结果
    bt2em = matrix(0,nrow = max.iter, ncol = M)
    ## 小h的迭代矩阵
    ...
    while( i<max.iter){
      ...
      exp.b1x = exp(Xp%*%bt1em[i,])
      LC.exp =  exp.b1x*Lambda1.Ci###V1i的值###
      ## 右删失部分的中间变量
      exp.b2x = exp(Xp%*%bt2em[i,])
      ...
      if(r1>0){
        ...
        exp.v2_v1bbv2<-tt6- exp.v1bbv2/L
        ...
        ##显示解##
        exp.1_phiv1bv2<- tt2-rowSums( as.matrix(exp(-HC.exp%*%t(bil))*r.mat(bil)*((1+r1*LC.exp%*%bil)^(-1/r1-1))))/L
        ...
        exp.1_phiv1bbv2<-tt6- rowSums( as.matrix(exp(-HC.exp%*%t(bil))*r.mat(bil^2)*((1+r1*LC.exp%*%bil)^(-1/r1-1))))/L
        ...
        ##############
        Ezi<-0
        Ezi3<-LC.exp*tt2/exp.v2_v1v2
        Ezi4<-LC.exp*tt6/exp.v2_v1bv2
      ...
      {
        ##BT<-c(0,0)
        Ezil.col/rowSums( rr.mat(exp(Xp%*%BT)*Ephibi)*t(bl.Ci) )
      }
      ...
      {
        #BT<-bt1em[i,]
        ss0 = rowSums( rr.mat(exp(Xp%*%BT)*Ephibi)*t(bl.Ci) )
        
        ss1 = rowSums( rr.mat(exp(Xp%*%BT)*Ephibi*Xp[,1])*t(bl.Ci) )
        ss2 = rowSums( rr.mat(exp(Xp%*%BT)*Ephibi*Xp[,2])*t(bl.Ci) )
        
        colSums((t(Ezil)%*%Xp)) - colSums((Ezil)%*%t(rbind(ss1,ss2)/matrix( rep(ss0,2),nrow = M,byrow = TRUE)))
      }
      ...
      es.hc = function(GM)
      {
        sdata$Delta/ as.numeric(t(exp(Xp%*%GM)*Ebi)%*%YCC)
      }
      
      
      
      ## 中间变量
      colsum.uZ = colSums(sdata$Delta*Xp)
      
      ## beta2的估计方程
      beta2.score = function(GM)
      {
        # GM<-bt2em[i,]
        exp.g.Eb = exp(Xp%*%GM)*Ebi
        ss0 = as.numeric(t(exp.g.Eb)%*%YCC)
        ss1 = as.numeric(t(exp.g.Eb*Xp[,1])%*%YCC)
        ss2 = as.numeric(t(exp.g.Eb*Xp[,2])%*%YCC)
        colsum.uZ - rowSums(sdata$Delta*(rbind(ss1,ss2)/matrix( rep(ss0,2),nrow = 2,byrow = TRUE)))
      }
      ...
      vfem[i+1] = max(1/htheta,1e-3)
      
      if(  all(sum(abs(bt1em[i+1,] - bt1em[i,]))<1e-2) 
           & all(sum(abs(bt2em[i+1,] - bt2em[i,]))<1e-2) 
    ...
    return(list(b10=c(bt1em[i+1,1],bt1em[i+1,2]),b20=c(bt2em[i+1,1],bt2em[i+1,2]), e0=vfem[i+1],gm0=gmem[i+1,],ap0=hcem[i+1,],iter=i+1) )   
  }  
